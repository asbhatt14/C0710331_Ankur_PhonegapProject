...
function getDatabase()
{
    ...
    try{
        ...
        {
            alert('Not supported -> Please try with Other browser');
        }
        else
        {
    ...
    $.ajax({
        ...
        success: function(json) {
            $(json.questions).each(function(){
                ...
                questions_json = this;
                quizQuestions.push(questions_json);
            });
...
function getQuestions_success(tx,results)
{
    ...
    for(var question_no=1;question_no<=len; question_no++)
    {
        ...
        var quest_div = "" +
            ...
            '<h1> Programming Concept Quiz</h1> ' +
            '</div>' +
        ...
        //    quest_div = quest_div + '<input type = "radio" name="answer" id="quest' + question_no + '-list" data-question-no="'+ question_no  + '" data-answer-no="' +  (i+1) + '">';
        //    quest_div =  quest_div  + '<label for="quest '+ question_no + '-list">' + answers[i] + '</label> <br>' ;
        }
        quest_div = quest_div +
            '</ul><p><br/></p>';
    //  '<p><br/></p>';
